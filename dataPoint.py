...
class dataPoint:
  ...
  def __init__(self, x, y, ph, tds, turbidity):
    ...
    self.values = numpy.asarray(self.values).reshape(1,-1)[0]
  ...
  def scale(self):
    rangeArray = numpy.asarray([14.0, 60906.259999999995, 5.29])
    return numpy.asarray([self.values[0]/rangeArray[0], self.values[1]/rangeArray[1], self.values[2]/rangeArray[2]])
  ...
  def normal(self):
    ...
    return newScaler.transform([self.values])
